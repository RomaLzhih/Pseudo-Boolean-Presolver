...
class SATPreSolver {
 public:
  ...
  void buildProblem();
  void redundancyDetection(papilo::Timer*& timer);
  void redundancyDetectionHeuristic(papilo::Timer*& timer);
  void hyperBinaryResolution(papilo::Timer*& timer);
  void presolve();
  void printSolution();
  void writePresolvers(const std::string& inFileName);
  const int& getPresolveStatus() { return this->presolveStatus; }
  void setOnlyPresolve(int& v) { this->onlyPreSolve = v; }
  void setLoggerPath(const std::string& lp) { this->loggerPath = lp; }
