...
void SATPreSolver<REAL>::buildProblem() {
...
template <typename REAL>
void SATPreSolver<REAL>::redundancyDetection(papilo::Timer*& timer) {
  ...
  while (D != es.end()) {
    if (timer->getTime() > this->timelimit) break;
    C = es.begin();
    while (C != es.end()) {
      if (timer->getTime() > this->timelimit) break;
      if (C == D || (C->getVarsSize() > this->redCmpSize && D->getVarsSize() > this->redCmpSize)) {
        C++;
        continue;
      }
      std::string preInfo = "* #variable= " + aux::tos(this->exprs.getVarNum()) + " #constraint= 2 \n" +
      ...
      rsStat = pre::runRoundingSat::runforRedundancy(preInfo, this->inputIns);
...
template <typename REAL>
void SATPreSolver<REAL>::redundancyDetectionHeuristic(papilo::Timer*& timer) {
  // msg.info( "running redundancy\n" );
...
template <typename REAL>
void SATPreSolver<REAL>::hyperBinaryResolution(papilo::Timer*& timer) {
  auto buildGraph = [&](const auto& es, Graph<REAL>& g) {
    ...
    bool flag = false;
    for (auto& e : es) {
      if (e.getVarsSize() != 2) continue;
      auto& cols = e.getCols();
      e.getLits(lits);
      assert(lits.size() == 2);
      if (cols.at(lits[0]) == 0 || cols.at(lits[1]) == 0) continue;
      if ((int)(cols.at(lits[0]) > 0) * cols.at(lits[0]) + (int)(cols.at(lits[1]) > 0) * cols.at(lits[1]) <
  ...
  auto resolveWithLit = [&](Expr<REAL>& e1, Expr<REAL>& e2, const int& u,
    ...
    REAL _lcm = aux::lcm(aux::abs(col1.at(u)), aux::abs(col2.at(u)));
    assert(_lcm > 0);
  ...
  auto resolve = [&](Graph<REAL>& g, const Expr<REAL>& longExp, Expr<REAL>& ansExp, const int& saveLit, int& v,
                     const int& gv) {
    std::vector<int> lits;
    ...
    for (auto& i : cols) {
      ...
      if (!commonLits.empty()) {
        for (auto& gv : commonLits)  // v is the common lit in graph
        {
          ...
          assert((!cols.count(v) && e.getVarsSize() == 2 && e.getCols().count(u) && e.getCols().count(v)) ||
...
template <typename REAL>
void SATPreSolver<REAL>::presolve() {
  auto setPara = [&]() {
    ...
    while (std::getline(parafile, line)) {
      ...
      else if (line.substr(0, line.find('=') - 1) == "redundancy.parallel")
  ...
  if (redDelNum || hbrAddedNum) this->presolveStatus = 1;
  ...
  if (onlyPreSolve) {
    if (this->presolveStatus == 1)  //* reduced
    {
      ...
      std::string midfile = this->inputIns.substr(0, this->inputIns.find_last_of('.')) + ".pre.opb";
  ...
  if (this->presolveStatus == 2) {
    ...
    rsSol = runRoundingSat::runforSAT(s, this->inputIns, RSTime);  // status, obj
  ...
  this->solutionStatus = rsSol.first == "UNSATISFIABLE" ? solStat::UNSATISFIABLE : solStat::SATISFIABLE;
  if (this->instanceType == fileType::opt && this->solutionStatus == solStat::SATISFIABLE) {
    assert(rsSol.second != "");
...
template <typename REAL>
void SATPreSolver<REAL>::printSolution() {
  std::cout << std::setw(16) << "BOOLEAN " << pbStatus << std::endl;  // 0 fail 1 pass
  // 0: opt, 1: dec
  std::cout << std::setw(16) << "FILE " << std::log2(utils::as_integer(this->instanceType)) << std::endl;
  // -1: already solved, 0: unchanged, 1: reduced,
  //  2: unbounded or infeasible, 3: unbounded, 4: infeasible
  std::cout << std::setw(16) << "PRESOLVE_STAT " << this->presolveStatus << std::endl;
  if (!this->onlyPreSolve) {
    // 0: UNSAT, 1:SAT
    std::cout << std::setw(16) << "SAT " << std::log2(utils::as_integer(this->solutionStatus)) << std::endl;
    if (this->solutionStatus != solStat::UNSATISFIABLE && this->instanceType == fileType::opt) {
...
void SATPreSolver<REAL>::writePresolvers(const std::string& inFileName) {
  ...
  std::cout << "$ " << inFileName.substr(inFileName.find_last_of("//") + 1);
  std::cout << " " << (this->solutionStatus == solStat::UNSATISFIABLE ? "0" : "1");
  std::cout << " " << (this->instanceType == fileType::opt ? aux::tos(this->origobj) : "-");
  std::cout << " " << this->RSTime << " " << this->redElapsedTime + this->hbrElapsedTime << " " << this->totalTime
            << std::endl;
  ...
  if (this->enablered) {
    ...
    std::cout << std::setw(16) << std::round(this->redElapsedTime * 10000) / 10000;
    std::cout << '\n';
  ...
  if (this->enablehbr) {
    ...
    std::cout << std::setw(16) << (this->hbrFindNum == 0 ? 0 : 1);  // succe
    std::cout << std::setw(16)                                      // tsx %
              << this->hbrFindNum;
    std::cout << std::setw(16)
              << (this->hbrFindNum != 0 ? 1.0 * this->hbrAddedNum / this->hbrFindNum : 0);  // tsxapplied
    std::cout << std::setw(16) << std::round(this->hbrElapsedTime * 10000) / 10000;
    std::cout << '\n';
  }
  return;
}
