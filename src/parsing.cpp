...
fileType
opb_read_to_papilo( std::ifstream& infile,
                    papilo::ProblemBuilder<REAL>& builder )
{
   ...
   int colNum =
       std::stoi( colNumStr.substr( 0, colNumStr.find_first_of( " " ) ) );
   int rowNum =
...
template <typename REAL>
fileType
opb_read_to_sat( std::ifstream& infile, ExprPool<REAL>& exprs )
{
   std::string line;
   std::getline( infile, line );
   std::string colNumStr = line.substr( line.find_first_of( "=" ) + 2 );
...
template fileType
opb_read_to_sat<bigint>( std::ifstream& infile, ExprPool<bigint>& exprs );
template fileType
opb_read_to_papilo<papilo::Rational>(
