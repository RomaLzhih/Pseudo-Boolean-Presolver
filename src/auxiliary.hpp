...
template <typename T, typename U>
std::ostream&
operator<<( std::ostream& os, const std::unordered_map<T, U>& m )
{
   for( const auto& e : m )
      os << e << ";" << std::endl;
   return os;
...
template <>
inline std::string
tos( const long long& x )
{
   return std::to_string( x );
}
template <typename T>
T
mod_safe( const T& p, const T& q )
{
   assert( q > 0 );
   if( p < 0 )
      return q - ( -p % q );
...
template <>
inline papilo::Rational
abs( const papilo::Rational& x )
{
...
template <>
inline boost::multiprecision::int128_t
abs( const boost::multiprecision::int128_t& x )
{
   return boost::multiprecision::abs( x );
}
...
template <>
inline boost::multiprecision::int128_t
gcd( const boost::multiprecision::int128_t& x,
...
template <>
inline boost::multiprecision::int128_t
lcm( const boost::multiprecision::int128_t& x,
     const boost::multiprecision::int128_t& y )
{
   return boost::multiprecision::lcm( x, y );
}
