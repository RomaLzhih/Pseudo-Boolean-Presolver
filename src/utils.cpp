...
void
printAbstractProblem( papilo::Problem<REAL> prob )
{
   ...
   // std::cout << "n continuous vars: " << prob.getNumIntegralCols() <<
   // std::endl;
   return;
}
...
template <typename REAL>
void
printDetailedProblem( papilo::Problem<REAL> prob )
{
   const papilo::ConstraintMatrix<REAL>& consmatrix =
       prob.getConstraintMatrix();
   const papilo::Vec<std::string>& consnames = prob.getConstraintNames();
   ...
   std::cout << "-------------- Print Problem ------------- " << prob.getName()
             << std::endl;
   ...
   for( int i = 0; i < consmatrix.getNRows(); ++i )
   {
      ...
      std::cout << "ROW " + std::to_string( i ) << "     LInf: "
                << row_flags[i].test( papilo::RowFlag::kLhsInf )
                << "    l: " << lhs[i] << "     RInf: "
                << row_flags[i].test( papilo::RowFlag::kRhsInf )
                << "    r: " << rhs[i] << std::endl;

      for( int j = 0; j < len; ++j )
      {
         std::cout << std::setw( 7 ) << varnames[indices[j]] << ":"
                   << int( rowVals[j] );
      }
...
template <typename REAL>
void
printVector( papilo::Vec<REAL> const& input )
{
...
template void
printAbstractProblem<papilo::Rational>(
    papilo::Problem<papilo::Rational> prob );
template void
printDetailedProblem<papilo::Rational>(
    papilo::Problem<papilo::Rational> prob );
template void
printVector<papilo::Rational>( papilo::Vec<papilo::Rational> const& input );
